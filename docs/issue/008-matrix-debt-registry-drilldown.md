...
GitLab の `filtered_search_bar` とラベル文化（`scope::value` の角丸ピル）、`vulnerability_report` の深刻度別一覧は **操作系の骨格** としてのみ借用する。DevDebtOps 固有の表現は次の通り：
...
- **担当者アバターは「理解している人」と「形式レビューだけの人」を視覚区別する。** `FileMastery.certified_via`（`quiz` / `authorship` / `review`）に応じてアバターの輪郭を変え、KC の高い理解者と「触ったが理解していない人」を一目で分ける。これは GitLab の単なる assignee アバターには無い、DevDebtOps のナレッジ負債観点そのものである。
...
- [ ] アクション（返済 PR 作成 / 無視 / 担当割当）の Coming Soon プレースホルダ表現を実装する
  - GitLab の空状態（empty-state）パターンを参考にしつつ、DevDebtOps ブランドの独自表現（後述）
...
- **`assigned_developers`（理解度つき）を `CodeDebt` にも持たせる。** §7.1 では `assigned_developers` は `KnowledgeDebt` のみだが、DevDebtOps の独自性（「理解者 / 形式レビューのみ」の視覚区別）を **コード負債側でも** 出すため、両種別に `assigned_developers`（`certified_via` + `coverage` つき）を持たせる。これは §7.1 の素朴な assignee には無い DevDebtOps 拡張である。
...
GitLab の empty-state パターン（中央配置のイラスト + 見出し + 補足 + 主アクション）を骨格として借りつつ、DevDebtOps ブランドの独自表現にする。
...
- 一覧 0 件 / 詳細未接続時の空状態は、DevDebtOps の「ロゼッタストーン解読中」モチーフ（`logo.svelte` ブランドアイコン + 解読中インジケータ）で、灰色一辺倒の GitLab empty-state とは差別化する。
