...
2. GitLab の枯れた Members IA（Manage グルーピング・末尾固定）を採用し、
   ナビ構造を学習コストの低いものにする。
3. DevDebtOps の一級概念である **Knowledge Coverage (KC)** を Settings にも通底させ、
...
純粋な権限管理テーブルである。DevDebtOps はここで一歩踏み込み、各メンバー行に
...
理解度を一級市民として扱う DevDebtOps の思想を、権限画面にも布石として埋め込む狙いである。
...
GitLab には無い DevDebtOps 独自列。本 issue では枠だけ用意し、Coming Soon を明示する。
