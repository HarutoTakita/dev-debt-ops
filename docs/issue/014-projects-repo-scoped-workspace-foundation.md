...
現在 DevDebtOps のルートはすべて **Org スコープ**（`/[org]/...`）で、Galaxy / Matrix / Quizzes / Agents /
...
DevDebtOps は次の点で意図的にずらす。

- **切り替えの主語を「観測対象リポジトリ」に再定義**する。GitLab の context switcher は group / project /
  organization という**所有・権限の階層**を渡り歩く道具だが、DevDebtOps のスイッチャーは
...
```
┌──────────────────────────────────────────────────────────────────────┐
│ Topbar:  [▤] [Λ DevDebtOps]   Org > Project > Matrix      [⌘K] [◍]       │
├──────────────┬───────────────────────────────────────────────────────┤
...
> 出すが、DevDebtOps は MVP では **localStorage の最近開いた順**に留める（サーバ側集計は Future）。
...
- GitLab 参考実装（`gitlab/`）— 構造を学び、世界観は独自化する
  ...
  - `app/assets/javascripts/super_sidebar/utils.js`（`trackContextAccess` — 最近項目の localStorage 追跡。DevDebtOps は軽量版）
