...
issue-004 で実装した ADK スタック解析の結果も表示する場所がない。DevDebtOps は仕様書 §6 で
...
持つが、DevDebtOps はその**情報設計と見た目**を意図的にずらす。

- **ナビ第一動詞を `understand` 系に再定義**する。GitLab は Repository / Merge requests / Pipelines という
  「コードを操作する」動詞順だが、DevDebtOps は Overview / Galaxy / Matrix / Quizzes / Agents / Learning を上位に置き、
  Repos（コード閲覧）は**参照用として末尾に格下げ**する。DevDebtOps の主語は「コードを書く」ではなく「コードを理解する」だから。
...
```
┌──────────────────────────────────────────────────────────────────────┐
│ Topbar:  [▤] [Λ DevDebtOps]   Org KC > Galaxy > 星系 > 星    [⌘K 検索] [◍]│
├──────────────┬───────────────────────────────────────────────────────┤
...
GitLab の `panel.rb` における `add_menu` 順 = 表示順、`render?` による表示制御を DevDebtOps では宣言的配列で表現する。
...
`isActive` を `(!isExpanded || isIconOnly) && is_active` で算出する。DevDebtOps では runes で同等にする。
...
<EmptyState title={`${feature} は近日公開`} {description}>
  {#snippet illustration()}
    <!-- 地層: アンバーの堆積層 + ティールの星を 1 つ。DevDebtOps 独自の Twin ビジュアル -->
...
```css
/* DevDebtOps Twin パレット — additive。shadcn 意味トークンは温存 */
@theme inline {
