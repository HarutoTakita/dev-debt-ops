...
Rosetta の価値は「二軸の技術負債を一目で把握できること」にある。仕様書 §6.1 のダッシュボードはプロダクトの顔であり、ここを最初に作り込むことで「Rosetta は何を見せるプロダクトか」を確定させる。
...
GitLab のダッシュボード（DevOps スコア・Value Stream・Security ダッシュボード）の主役は、いずれも **open Issue / MR の件数リスト** や **指標カードの格子** である。Rosetta はこれを反転させる。
...
- **地層メタファー**：推移は GitLab の緑の草グラフ（積み上げ折れ線）ではなく、**地層断面**として描く。Rosetta のブランド語彙「地層 = 負債が堆積する層」（仕様書 §1）を可視化に直結させ、理解の堆積を地層の積み重なりで表現する。
...
- [ ] `<svelte:head>` のタイトルを「観測台 · Rosetta」に変更する
...
- [ ] `frontend/src/lib/components/overview/coming-soon-placeholder.svelte` を実装する（GitLab の空状態パターンを参考にした Rosetta ブランド独自表現。下記「技術詳細」参照）
...
GitLab の `trend_indicator.vue` は `trendStyle` で色を決める：`asc` は上昇=success、`desc` は上昇=danger。Rosetta では負債系指標に `desc` を使い、**減少を success（緑）** として表現する。
...
### Coming Soon プレースホルダ（Rosetta ブランド独自表現）

GitLab の空状態は `gl-empty-state`（中央のイラスト + 見出し + 説明 + CTA）。これを参考にしつつ、Rosetta では **地層メタファー** を流用した独自表現にする。
...
- GitLab 参考実装（独自性のために**写像**として参照、丸パクリはしない）：
  - `/Users/takitaharuto/tech-debt-agent/gitlab/app/assets/javascripts/analytics/dashboards/components/trend_indicator.vue` — `trendStyle`（`asc`/`desc`）による色反転。Rosetta は負債=減少を success に反転利用
