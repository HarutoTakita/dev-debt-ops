...
現状 `frontend/src/routes/[org]/+page.svelte` は `<title>Dashboard · DevDebtOps</title>` を名乗りながら、中身は丸ごとリポジトリビューア（`RepoPicker` / `TechStackPanel` / `FileTree` / `FileViewer`）という **情報設計（IA）の混線** を抱えている。本 issue ではこれを解消し、あわせてビューア各部を GitLab の repository コンポーネント群を参考に磨き込む。具体的には次の 4 点を行う。
...
GitLab を参考にするのはレイアウトの所作（パンくず・段階ロード・種別別ビューア）までで、DevDebtOps の核は別にある。GitLab のツリーは行数バッジ（LOC）や CI ステータスを並べるが、DevDebtOps は同じ枠を **負債密度バッジ** に転用する。`FileTree` の各ノードに「行数バッジ枠」ではなく「負債密度バッジ枠」を最初から仕込んでおき、後続の二軸負債マトリクス（仕様書 §2.3）や Knowledge Coverage（§5.1）と **共通言語** で繋ぐ。同様に `FileViewer` は GitLab の赤緑 diff ビューアには寄せず、ソース層の上に **知識オーバーレイ層（KC ヒートマップ行 + 考古学注釈）** を重ねられる二層構造の土台にする。配色も GitLab 紫ではなく、DevDebtOps ブランドの **地層アンバー系** で統一する（負債は「埋もれた地層」というメタファ）。本 issue では負債データ自体は流し込まないが、枠と二層構造だけは先に用意しておく。
...
- [ ] `<svelte:head>` の `<title>` を `Repos · DevDebtOps` に変更する
...
- [ ] `<title>` を `Dashboard · DevDebtOps` のまま維持し、中身を **Overview プレースホルダ** に置き換える（仕様書 §6.1 の二軸マトリクス / 推移グラフ / 優先対応リストが入る場所であることを示す軽い案内 + `Repos へ` への導線）。Overview 本体の実装は別 issue
...
- `/[org]`（Overview）にはリポジトリビューアが一切表示されず、Overview プレースホルダと `Repos へ` の導線のみが表示されること（`<title>` は `Dashboard · DevDebtOps`）
...
GitLab の `loadViewer(type, isUsingLfs, isTooLarge)` 写像に倣い、DevDebtOps は軽量に種別を解決する。動的 import ではなく単純な分岐で十分。
