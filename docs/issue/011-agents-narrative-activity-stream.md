...
**受動的 Q&A** である。Rosetta はこれを **反面教師** とする。Twin Agent は右下チャットを
...
を表す。会話 UI ではなく **自律的行動の可視化** が中心であることが Rosetta の差別化点である。
...
- [ ] `frontend/src/lib/components/ui/coming-soon-placeholder.svelte` を作成する
      （Rosetta ブランドの空状態。後続 issue でも再利用可能な汎用コンポーネント）
...
- **機能本体（実エージェント連携・実 PR/クイズ生成）は実装せず**、未配線領域に
  `ComingSoonPlaceholder`（Rosetta ブランドの空状態）が表示されること
...
のと同様、`AgentStatus` を Rosetta 独自のアイコン + 色に写像する。アイコンは既存依存の
...
**構造** を借りつつ、見た目は Rosetta ブランドで独自に表現する。汎用コンポーネントとして
...
ブランド独自表現の指針: GitLab の白背景 + グレーイラストではなく、Rosetta の
