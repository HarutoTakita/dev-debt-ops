...
Rosetta は GitLab の `contributors.vue` / `individual_chart.vue`（個人別コミット頻度チャート）から
...
- [ ] `coming-soon-placeholder.svelte`（`frontend/src/lib/components/galaxy/coming-soon-placeholder.svelte`）を実装する
  - GitLab の `GlEmptyState` パターンを参考にしつつ Rosetta ブランドの独自表現（星域メタファー）にする
...
- [ ] `[org]/galaxy/+page.svelte` を新規作成する
  ...
  - `<svelte:head>` に `<title>Knowledge Galaxy · Rosetta</title>`
...
│   ├── coming-soon-placeholder.svelte     # 星域未観測の空状態（Rosetta ブランド）
...
Rosetta の他空状態（`repo-picker.svelte` の「GitHub App をインストール」等）と同じ中央寄せレイアウトに揃える。
...
<svelte:head><title>Knowledge Galaxy · Rosetta</title></svelte:head>
