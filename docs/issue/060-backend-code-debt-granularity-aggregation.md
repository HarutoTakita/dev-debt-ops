...
`code_debt_score` の max を流用**するに留まった（ADR `docs/adr/0006`）。本 issue はコード負債
...
- [ ] 集約方針（max / 件数 / 加重）の ADR 追記（0006 を更新 or 新規）。
...
- `backend/api/app/services/debt_query.py`（`_node_from_files` / `build_overview`）、`code_debts`、
  `feature_files`、ADR `docs/adr/0006-feature-kc-rollup.md`、057 / 055。
