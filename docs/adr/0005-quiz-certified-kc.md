...
## 機能スコープ受験の展開（issue 054 で追記）

`granularity="feature"` のクイズセッション（issue 054）は、採点後に **機能配下の全ファイルへ
スコアを一律展開**して各ファイルの `file_kc` を更新する（MVP）。`feature_files.confidence`
による加重展開は将来検討。`quiz_results.kc_before`/`kc_after` は代表ファイル（最初に反映した
ファイル）の値を要約として用いる。機能単位 KC の実体はファイル KC のロールアップ（055 が集計）。

## 対象外

- 機能（feature）単位の KC 認定の集計・配信 → 055。
