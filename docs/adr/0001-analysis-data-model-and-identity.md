...
- 解析データは project 単位（1 project = 1 repo）。`analysis_run.project_id` を全解析データの親スコープとする。
  ただし `projects` は api 所有テーブルで `shared` の metadata に無いため、`Job.project_id` と同じく **ORM/DDL レベルの FK は張らず索引のみ**とする（FK を張ると service の `SQLModel.metadata.create_all` が projects を解決できず `NoReferencedTableError` になる）。join は索引で行い、整合性はアプリ層で担保する。`job_id` → `jobs.id` は shared 内で解決できるため FK を張る。
