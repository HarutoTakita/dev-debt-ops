...
### インフラ構成 — スケーラブル & セキュアなサーバーレス基盤

Google Cloud を活用した、フルスタック・サーバーレス構成です。
コンテナ化したアプリを **Cloud Run** で自動スケールさせ、負荷の高い解析は **Cloud Tasks** で非同期ワーカーへ切り出し、シークレットや通信は **Secret Manager・Cloud Armor・Workload Identity Federation** で保護、インフラ全体は **Terraform** でコード化——マネージドサービスを全面採用することで、**スケーラビリティ・セキュリティ・可用性**を高い水準で両立した構成を実現しています。
...
### 実運用を見据えたデプロイ戦略 — ゼロダウンタイム & 即時ロールバック

**「止めずに・安全に・すぐ戻せる」** リリースを実現するため、デプロイ作業も DevOps のプラクティスに基づいて設計しています。

- **環境分離（staging / production）**
  - `develop` へのマージで **staging 環境**へ自動デプロイし、本番に近い構成で統合検証。
  - `v*.*.*` タグの push で **production 環境**へのリリースデプロイを行う。
- **ブルーグリーンデプロイメント**
...
### システムの中核となる AI エージェント — 自律的な Agentic アーキテクチャ
...
### AI 時代の DevOps — 品質を自動で担保する開発サイクル
...
### サービス化を見据えたプロダクトとしての完成度

個人が使うツールで終わらせず、**チーム・組織に提供する SaaS** としての実用性を見据え、プロダクトの完成度を高めています。

- **多言語対応（i18n）** — Paraglide による **6 言語**（日本語・英語・中国語・韓国語・スペイン語・ドイツ語）対応。海外メンバーを含むチームや OSS でもそのまま利用できます。
- **アカウント / ユーザー管理** — GitHub SSO でのログインに加え、アカウント設定・ユーザー管理を提供。チームでの利用を前提とした基盤を備えています。
- **公開に耐える体裁** — ランディングページに加え、**プライバシーポリシー**や**利用規約**を整備し、外部公開・サービス提供に必要な要件を満たします。
- **可観測性** — **Cloud Monitoring / Logging** で 5xx エラー率や uptime を継続監視し、SaaS 運用に不可欠な監視体制を確保しています。

### 拡張性・保守性を見据えたドキュメントの自動整備

拡張性・保守性を見据えて、**リリースと同時に主要ドキュメントが自動で最新化される仕組み**を組み込んでいます。
ドキュメントが常にコードへ追従するため、機能追加やチーム拡大にも耐える**保守性・拡張性**を確保しています。

- **`/release-version` で新バージョンを切ると、以下が自動更新されます。**
  - **API ドキュメント** — コードから **OpenAPI 3.1**形式のAPIドキュメント（[`docs/reference/openapi.json`](https://github.com/HarutoTakita/dev-debt-ops/blob/main/docs/reference/openapi.json)）を再生成。
  - **ER 図（DBML）** — 実際の DB スキーマから DBML形式のER図（[`docs/reference/schema.dbml`](https://github.com/HarutoTakita/dev-debt-ops/blob/main/docs/reference/schema.dbml)）を再生成。
  - **取扱説明書** — Playwright を用いて、アプリ画面のスクリーンショットを自動で取り直し、取扱説明書（[`docs/user-guide/README.md`](https://github.com/HarutoTakita/dev-debt-ops/blob/main/docs/user-guide/README.md)）の本文をAIが更新。
  - **チェンジログ** — マージ済み PR / コミットから変更履歴（[`CHANGELOG.md`](https://github.com/HarutoTakita/dev-debt-ops/blob/main/CHANGELOG.md)）を自動生成。
