...
## 使い方アシスタント（デモ・チャット UI）

右下のフローティングボタンから開くチャットパネル（`index.html` の `#chat-launcher` / `#chat-panel`）。
アプリの使い方の質問に答え、改善要望は問い合わせメール（フッターと同じ `mailto`）へ誘導する。

- **実際の LLM 呼び出しは未実装**。応答はキーワード意図マッチの**ルールベース簡易応答**（`index.html` 末尾の
  IIFE 内 `INTENTS` / `A`）。パネル上部に「デモ機能・本番リリース後に AI 導入予定」の但し書きを常時表示する。
- バックエンド/ネットワーク呼び出しなし（`fetch` を使わない）。LP 単体で完結。
- i18n: 表示テキストは `data-i18n="chat.*"`（`i18n/*.json` の 6 言語＋`i18n/build.py` で再生成）。回答本文は
  スクリプト内の locale 別マップ（`PARAGLIDE_LOCALE` を参照）。placeholder/aria はスクリプトで設定。
- 新しい Tailwind クラスを足したら **`npm run build:css` で `styles.css` を再生成**すること。
