...
## バックエンド (`backend/` — uv workspace モノレポ)
- **構成:** `backend/` が workspace ルート。メンバーは `shared`（共有 enum・pydantic スキーマ・キュー/Blob Protocol・ORM モデル `Job`）/ `api`（既存 FastAPI。`app.*` 名前空間は不変。外部公開 Cloud Run）/ `service`（重い処理 worker。Cloud Tasks の `/tasks/{pipeline}` HTTP ターゲット。内部 Cloud Run）。`shared` のみ実パッケージ（hatchling）として install され `from shared.models import Job` 等が両者から可能。`api` / `service` は `[tool.uv] package = false`（依存のみ install しソースから実行）。
- **スタック:** FastAPI + SQLModel + SQLAlchemy 2.0 async + PostgreSQL + Alembic
- **Python:** 3.13、`uv` で管理（全メンバーを 1 ロック `backend/uv.lock` に解決。`uv sync --all-packages`）
- **開発サーバー:** `docker compose watch` (api を :8000、service、db を起動しコード同期)
- **テスト:** `cd backend && uv run --directory api pytest`（`shared` / `service` も `--directory <member>`。api は Postgres の `test` DB が必要）
...
docker compose watch             # db + api (:8000) + service（コード同期）
docker compose --profile tools up  # pgAdmin を :5050 で含める（オプション）
cd frontend && bun run dev       # ネイティブVite HMR on :5173, /api を :8000(api) にプロキシ
```

### 本番モードローカルスタック (3レプリカ + Traefik)
```
docker compose -f compose.prod.yml up --build   # api(3レプリカ) + service(2レプリカ) + Traefik on :8080
open http://localhost:8080                      # Traefik → api がビルド済みSPAと/apiの両方を配信
```
Traefikダッシュボードは :8081。`docker/api.Dockerfile`（runtime ステージ）が frontend ステージで `bun run build` を実行し、静的バンドルを `app/static` にコピー、`main.py` が `ENVIRONMENT=prod` の時に `/` にマウントする。`service` は Traefik では公開せず（本番は Cloud Run internal + Cloud Tasks）、api から内部ネットワーク経由で到達する。
...
- **GitHub Actions** (`.github/workflows/ci.yml`): push + 非draft PRで実行
  - `check-backend` — shared / api / service の ruff check + format + ty check（`uv sync --all-packages`）
  - `test-backend` — shared / api / service の pytest（api は Postgres 17 サービスコンテナ + `test` DB）
