  ...
  {:else}
    ...
    <div class="mt-3">
      <div class="relative h-40">
        <!-- 50% の目安線 -->
        <div class="pointer-events-none absolute inset-x-0 top-1/2 border-t border-dashed border-border/60"></div>
      ...
      <div class="mt-1 flex gap-1.5">
        {#each trend as p (p.week)}
          <span class="min-w-0 flex-1 truncate text-center text-[9px] text-muted-foreground">{weekLabel(p.week)}</span>
        {/each}
      </div>
    </div>
  {/if}
