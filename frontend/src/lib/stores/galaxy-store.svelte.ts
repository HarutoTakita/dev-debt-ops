...
class GalaxyStore {
  ...
  // ファイル単位グラフ（issue 288）のエッジ = CodeGraphContext の file↔file 結合（実コール集約）。
  // 未構築/取得失敗時は空＝star-map が galaxy.wormholes（import グラフ）にフォールバックする。
  ...
  // 実 API（issue 032）: 最新 kc_analysis run を投影して取得。あわせて code-graph（file_edges, issue 288）も
  // 並行取得（失敗しても galaxy 表示は止めず、wormhole にフォールバック）。未観測でも代入する。
