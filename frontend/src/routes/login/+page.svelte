...
<svelte:head>
  <title>{m.login_title()} · DevDebtOps</title>
</svelte:head>
...
<div class="relative flex min-h-screen flex-col items-center justify-center gap-8 overflow-hidden p-8">
  ...
  <div class="relative z-10 flex flex-col items-center gap-3">
    ...
    >
      ...
      {loading ? m.login_redirecting() : m.login_github()}
    </button>
    ...
    {#if demoEnabled}
        ...
        <Tooltip.Root>
          <Tooltip.Trigger>
            {#snippet child({ props })}
              ...
              >
                {demoLoading ? m.login_preparing() : m.login_demo_cta()}
              </button>
            {/snippet}
          </Tooltip.Trigger>
      ...
      {#if demoError}
        <p class="text-xs text-red-400">{m.login_demo_error()}</p>
      {/if}
    {/if}
  </div>
</div>
