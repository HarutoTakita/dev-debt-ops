"""``GcsBlobClient`` — Cloud Storage implementation of the ``BlobClient`` Protocol.

``google-cloud-storage`` is imported lazily so that importing this module (and running
the mock-path tests) never requires the GCS client or credentials. Blocking GCS calls
run in a thread via ``asyncio.to_thread`` to keep the event loop free.
"""
...
class GcsBlobClient:
    ...
    async def upload(self, bucket: str, object_path: str, data: bytes, content_type: str = "application/json") -> str:
        ...
        def _upload() -> None:
            blob = self._get_client().bucket(bucket).blob(object_path)
    ...
    async def delete(self, bucket: str, object_path: str) -> None:
        """Delete ``gs://bucket/object_path`` (ignore if absent)."""

        def _delete() -> None:
            self._get_client().bucket(bucket).blob(object_path).delete()

        with contextlib.suppress(Exception):  # delete is best-effort
