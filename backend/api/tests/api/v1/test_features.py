"""api feature clustering: ``POST .../cluster-features`` enqueues (202) without running it.

Mirrors the analyze-kc / analyze-stack enqueue tests: installation-id is stubbed, the mock
...
async def test_cluster_features_enqueues_and_returns_202(authenticated_client: AsyncClient) -> None:
    org_slug, project_slug = await _seed_project(authenticated_client)
    resp = await authenticated_client.post(f"/api/v1/orgs/{org_slug}/projects/{project_slug}/cluster-features")
    ...
    async with app_db.async_session_maker() as session:
        ...
        assert job.job_type == JobType.FEATURE_CLUSTERING
    ...
    assert dispatcher.tasks[0].pipeline == JobType.FEATURE_CLUSTERING.value
