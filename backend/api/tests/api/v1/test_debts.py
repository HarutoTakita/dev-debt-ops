"""api code-debt detection: ``POST .../detect-debts`` enqueues (202) without running analysis.

The heavy detection runs in the ``service`` container (issue 028); api only enqueues.
...
async def test_detect_debts_enqueues_and_returns_202(authenticated_client: AsyncClient) -> None:
    org_slug, project_slug = await _seed_project(authenticated_client)

    resp = await authenticated_client.post(f"/api/v1/orgs/{org_slug}/projects/{project_slug}/detect-debts")
    assert resp.status_code == 202
    body = resp.json()
    assert body["status"] == "QUEUED"
    job_id = uuid.UUID(body["job_id"])

    # Method-B payload: installation_id only (no secret), project_id + repo carried.
    async with app_db.async_session_maker() as session:
        job = await session.get(Job, job_id)
        assert job is not None
        assert job.job_type == JobType.CODE_DEBT_DETECTION
        assert job.status == JobStatus.QUEUED
        assert job.payload["owner"] == "acme"
        assert job.payload["repo"] == "rosetta"
    ...
    # Dispatched exactly once for the code_debt_detection pipeline.
    dispatcher = get_task_dispatcher()
    assert len(dispatcher.tasks) == 1
    assert dispatcher.tasks[0].pipeline == JobType.CODE_DEBT_DETECTION.value
