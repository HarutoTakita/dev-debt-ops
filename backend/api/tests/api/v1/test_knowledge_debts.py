"""api knowledge-debt detection: ``POST .../detect-knowledge-debts`` enqueues (202).

The heavy detection runs in the ``service`` container (issue 030); api only enqueues.
"""
...
async def test_detect_knowledge_debts_enqueues_and_returns_202(authenticated_client: AsyncClient) -> None:
    org_slug, project_slug = await _seed_project(authenticated_client)

    resp = await authenticated_client.post(f"/api/v1/orgs/{org_slug}/projects/{project_slug}/detect-knowledge-debts")
    ...
    async with app_db.async_session_maker() as session:
        ...
        assert job.job_type == JobType.KNOWLEDGE_DEBT_DETECTION
    ...
    assert dispatcher.tasks[0].pipeline == JobType.KNOWLEDGE_DEBT_DETECTION.value
