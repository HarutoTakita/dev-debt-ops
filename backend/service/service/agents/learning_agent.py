...
機能名・説明・構成ファイル一覧、そして主要ファイルのコード抜粋（=== path === 区切り）があります。**実際の
コード抜粋に基づき**、コードを読む順に学習ステップを作ってください。機能を構成する**主要な複数ファイルを横断**
して説明し（主要ファイルごとに最低 1 ステップ設け、特定の 1 ファイルに偏らせない）、各ステップで対象ファイルの
