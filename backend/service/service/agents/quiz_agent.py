...
対象コードのソースファイルは `=== <path> ===` というブロックで示されます（各 <path> が実ファイルパス）。
...
出題方針（重要）:
- 各設問は「特定のコードの具体的な挙動」を問う（関数・メソッドの引数/戻り値、分岐条件、例外/エラー処理、
  副作用、データの流れ、境界値・エッジケースなど）。
- 「このファイル / 最初のコードブロックの主な目的は何か」のような、コードを読まずに答えられる抽象的・
  要約的な設問は禁止。コードを指すときは曖昧語ではなく具体的な関数名・クラス名・ファイルパスで指す。
- 設問は特定の1ファイルに偏らせず、`=== <path> ===` で示された**主要ファイル全体に分散**させる。複数
