...
- 製品の「機能（feature）」の候補と所属ファイル、各機能の学習上の重要概念（key concepts）。
  機能は**ユーザー/プロダクト視点の能力**（例: リポジトリ解析、理解度クイズ、学習プラン生成、コード品質検知、
  GitHub 連携、認証、ダッシュボード）で捉える。**フォルダ名やレイヤー**（バックエンド / フロントエンド /
  API 層 / UI 層 / パイプライン / 設定 / ユーティリティ / テスト）**で分けない**。リポジトリ全体を覆うよう
  **十分な数**（大きめのリポジトリなら 10〜20 個目安）を挙げ、見えたソースはできるだけどれかの機能に対応づける。
...
- features[]: {{"key": "小文字 kebab/snake の安定 slug（英語可）",
  "name": "ユーザー視点の機能名（例: リポジトリ解析、理解度クイズ、学習プラン生成、GitHub 連携、認証）",
  "description": "1〜2 行（日本語）",
...
ルール:
- パスは探索で実在が確認できたものだけを使う。数値は入れない（判断・分類・根拠のみ）。
- 機能は**ユーザー/プロダクト視点の能力**で命名・分類する。**フォルダ名やレイヤー**（バックエンド /
  フロントエンド / API 層 / UI 層 / パイプライン / 設定 / ユーティリティ / テスト）**で分けてはいけない**。
  「バックエンド API」「フロントエンド UI」のような層名は禁止。
- リポジトリ規模に見合う**十分な数**の機能に分ける（大きめのリポジトリなら 10〜20 個目安）。1〜数個の
  大きな塊にまとめない。探索で見えたソースは**できるだけどれかの機能へ割り当てる**（大半を未分類で残さない）。
- 機能名（name）・説明（description）は**必ず自然な日本語**にする
